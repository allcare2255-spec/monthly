...
export function PreStep({ n }: { n: number }) {
  return (
    <>
      <StepHeader
        n={n}
        title="첫 컨설팅 전에, 사전 질문지를 먼저 받아요"
        desc="목표 대학, 지금 성적, 과목별 공부 방식과 고민을 미리 적어 보내요. 성실하게 작성할수록 첫 컨설팅이 학생에게 딱 맞게 준비돼요."
      />
      <Points
        items={[
          { icon: "🎯", title: "목표 확인", text: "목표 대학·학과와 필요한 성적을 적어요" },
          { icon: "📊", title: "현재 상태", text: "과목 선택과 지금 성적대를 알려줘요" },
          { icon: "📝", title: "과목별 고민", text: "공부 방식과 문제점을 과목마다 적어요" },
        ]}
      />
      <PreForm />
    </>
...
export function FirstZoomStep({ n }: { n: number }) {
  return (
    <>
      <StepHeader
        n={n}
        title="첫 컨설팅에서 4주 방향을 함께 정해요"
        desc="멘토가 사전 질문지를 꼼꼼히 읽고 줌으로 1:1 첫 컨설팅을 해요. 지금 공부 습관을 함께 점검하고, 과목별로 4주 동안 무엇을 어떻게 할지 정해요."
      />
      <Points
        items={[
          { icon: "🔍", title: "공부 습관 점검", text: "사전 질문지를 화면에 띄워 함께 봐요" },
          { icon: "🧭", title: "과목별 방향", text: "과목마다 먼저 바꿀 공부법을 정해요" },
          { icon: "🗓️", title: "4주 목표 설정", text: "4주 동안 이룰 목표와 매일 할 과제를 정해요" },
        ]}
      />
      <ZoomMock
        badge="첫 컨설팅"
...
// ─────────────────────────────────────────────────────────────
// 멘토 · 매니저 · 고예찬 대표 3인 관리 (TEAM 이 있을 때만)
// ─────────────────────────────────────────────────────────────
  ...
  return (
    <>
      <StepHeader
        ...
        desc="담당 멘토 혼자가 아니라, 매니저와 고예찬 대표가 함께 한 학생의 코칭을 챙겨요. 공부는 멘토가, 매일의 관리는 매니저가, 전체 방향과 전용 자료는 고예찬 대표가 맡아요."
      />
